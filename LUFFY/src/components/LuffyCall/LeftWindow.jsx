...
/**
 * LeftWindow 컴포넌트는 화상 통화 기능을 제공하는 UI입니다.
 * 
 * @component
 * @param {object} props - 컴포넌트에 전달되는 props
 * @param {object} props.meetingInfo - 회의 정보
 * @param {function} props.send - 메시지를 보내는 함수
 * @param {function} props.updateLocalVideo - 로컬 비디오 업데이트 함수
 * @param {function} props.readyCam - 카메라 준비 함수
 ...
 * @param {object} props.camContainer - 카메라 컨테이너 ref
 * @param {object} props.remoteCam - 원격 카메라 ref
 * @param {object} props.localCamContainer - 로컬 카메라 컨테이너 ref
 * @param {object} props.localCam - 로컬 카메라 ref
 * @param {function} props.updateRemoteVideo - 원격 비디오 업데이트 함수
 * @param {function} props.stop - 멈추는 함수
 * @param {function} props.endCall - 통화 종료 함수
 */
function LeftWindow({
  meetingInfo,
  send,
  ...
  const { userInfos } = useCounselStore();
  const { userId } = useUserStore();

  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (meetingInfo.connect.offerState === 1) {
        send({
          event: "request loading",
        });
        // Set voice recognition to Korean and process the results once the user finishes speaking
        listen({ interimResults: false, lang: "ko-KR" });
      }
    }, userInfos[0].id === userId ? 1000 : 3000);

    return () => clearTimeout(timeoutId);
  }, [meetingInfo.connect.offerState, send, listen, userInfos, userId]);
              ...
              {meetingInfo.connect.offerState === 1 ? (
                <button
                  ...
                  onClick={(event) => {
                    ...
                    // 음성인식을 한국어로 설정하고 결과를 말하는 게 끝나면 한꺼번에 받아오는 방식으로 변경
                    listen({ interimResults: false, lang: "ko-KR" });
                  }}
                >
                  <img src={phoneOnImg} />
